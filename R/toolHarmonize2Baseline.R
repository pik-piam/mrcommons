...
toolHarmonize2Baseline <- function(x, base, ref_year = "y2015", method = "limited", hard_cut = FALSE) {
  ...
  if (hard_cut) {
    ...
    full[, after_ref, ]       <- x[, after_ref, ] * (base[, rep(ref_year, length(after_ref)), ] / x[, rep(ref_year, length(after_ref)), ])
