...
calcEmisNitrogenOceans <- function(method = "ACCMIP") {
  ...
  if (method == "ACCMIP") {
    emi <- collapseNames(calcOutput("AtmosphericDeposition", datasource = "ACCMIP", glo_incl_oceans = FALSE,
                                    cellular = FALSE, emission = TRUE, aggregate = FALSE))
    emiGlo <- collapseNames(calcOutput("AtmosphericDeposition", datasource = "ACCMIP", glo_incl_oceans = TRUE,
                                       cellular = FALSE, emission = TRUE, aggregate = FALSE))
    out <- emi
    out[, , ] <- 0
    out["ATA", , ] <- emiGlo - dimSums(emi, dim = 1)
    ...
    # Voss, M., H. W. Bange, J. W. Dippner, J. J. Middelburg, J. P. Montoya, and B. Ward. 2013.
    # "The Marine Nitrogen Cycle: Recent Discoveries, Uncertainties and the Potential Relevance of Climate Change."
    # Philosophical Transactions of the Royal Society B: Biological Sciences 368 (1621): 20130121–20130121.
    # doi:10.1098/rstb.2013.0121. #nolint: commented_code_linter.
    ...
    baseyear <- "y2000"
    if (method == "Nsurplus2") {
      budget <- collapseNames(calcOutput("NitrogenBudgetOcean", deposition = "Nsurplus",
                                         aggregate = FALSE)[, , "surplus"])
    } else {
      budget <- collapseNames(calcOutput("NitrogenBudgetOcean", deposition = "CEDS", leaching = "Nsurplus",
                                         aggregate = FALSE)[, , "surplus"])
    }
