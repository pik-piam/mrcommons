...
readAdjustGrassi2021 <- function(subtype) {
  ...
  if (subtype == "data") {
    ...
    for (rcp in (c("RCP19","RCP26","RCP34","RCP45","RCP60","RCPBU"))) {
      inter <- read.table(paste0("Grassi_et_al_2021_adjustment_factors_",rcp,".csv"),sep=";",header = TRUE,check.names = F)
  ...
  if (subtype == "weight") {
    w_table <- read.table("Grassi_et_al_2021_country_removals.csv",sep=";",header = TRUE,check.names = F)
