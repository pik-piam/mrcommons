...
calcNitrogenBudgetCropland <- function(cellular = FALSE,
  ...
  fixation <- dimSums(calcOutput("NitrogenFixationBNF", sum_plantparts = TRUE,
                                 aggregate = FALSE, cellular = cellular), dim = 3.2)
  fixation <- dimSums(calcOutput("NitrogenFixationFreeliving", aggregate = FALSE, cellular = cellular), dim = 3.2)
  ...
  inputsDirect <- mbind(setNames(seed, "seed"), setNames(fixation[, , "fixation_crops"], "fixation_crops")
  )
