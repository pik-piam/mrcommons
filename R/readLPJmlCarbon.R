...
readLPJmlCarbon <- function(subtype = "historical.nat_veg.soilc_layer") {
  ...
  filename    <- paste0(gsub("\\.", "/", subtype), ".bin")
  typeRaw     <- subtype
  subtype     <- gsub("^.+\\.", "", typeRaw)
  landtype    <- gsub("^.+\\.", "", gsub(paste0(".", subtype), "", typeRaw))
  climatetype <- gsub(paste0(".", landtype, ".", subtype), "", typeRaw)


  lpj2mag <- c(lu_maize_nres_if = "crop",
  ...
  landtype <- toolSubtypeSelect(landtype, lpj2mag)
  ...
  if (grepl("historical", climatetype)) {
    ...
    stop(paste0("startYear and range of ", climatetype, " unknown"))
  }
  ...
  if (subtype %in% c("soilc", "litc", "vegc", "alitfallc")) {
    ...
    x <- add_dimension(x, dim = 3.1,
                       add = "climate.landuse",
                       nm = paste0(climatetype, ".", landtype))
