...
readFAO <- function(subtype) {
  ...
  if (subtype == "ForestProdTrade") {
    ...
    FAO <- read.table(file, header = FALSE, skip = 1, sep = ",", colClasses = readcolClass, col.names = csvcolnames, quote = "\"", encoding = "latin1")
    ...
    FAO <- fread(input = file, header = FALSE, skip = 1, sep = ",", colClasses = readcolClass, col.names = csvcolnames, quote = "\"", encoding = "Latin-1", showProgress = FALSE)
