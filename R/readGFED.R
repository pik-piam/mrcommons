...
readGFED <- function() {
	...
	.calculate_year <- function(year, emission, partition)
	{
		H5D_data <- h5file(filename = paste0("GFED4.1s_", year, ".hdf5"))

		# Sum the emissions across months for this emission|partition
		month_ids <- c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
		year_total <- matrix(nrow = 1440, ncol = 720, data = 0)
		walk(.x = month_ids,
			 .f = ~
			 	{
			 		# Get total DM emissions
			 		DM_emissions_id <- paste0("emissions/", .x, "/", emission)
			 		DM_emissions <- H5D_data[[DM_emissions_id]]$read()

			 		# Get the partition's contributions
			 		partition_id <- paste0("emissions/", .x, "/partitioning/", emission, "_", partition)
			 		partition_fraction <- H5D_data[[partition_id]]$read()

			 		year_total <<- year_total + (DM_emissions * partition_fraction) # Note scoped assignment
			 	})

		# kg DM m^-2 to kg DM
		sq_meters_per_gridcell <- H5D_data[["ancill/grid_cell_area"]]$read()
		year_total <- year_total * sq_meters_per_gridcell

		# kg DM to Mt DM
		year_total <- year_total * 1E-09
	...
	available_years	<- list.files(pattern = "\\d{4}") %>% str_extract("\\d{4}")

	d <- expand_grid(year = available_years,
					 base_emission = "DM",
					 partition = partitions_considered)
	...
	d <- d %>%
		mutate(DM = pmap(.l = list(.data$year, .data$base_emission, .data$partition),
						 .f = ~ .calculate_year(..1, ..2, ..3)))

	# Read in further emission factors
	emission_factors <- read.table("GFED_emissionsFactors.txt",
	...
	d <- inner_join(d, emission_factors, by = "partition") %>%
