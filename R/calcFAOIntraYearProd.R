...
#
  ...
  cropcal <- readSource("GGCMICropCalendar", subtype = "cal", convert = FALSE)[, , day][, , "rf"]
  ...
  area_mask <- calcOutput("Croparea", cellular = TRUE, aggregate = FALSE)[, 2010, ]
  area_mask <- ifelse(area_mask > 0, 1, 0)
  area_mask <- area_mask[, , c("begr", "betr", "foddr", "oilpalm", "others"), inv = TRUE]

  ggcmi_mapping <- toolGetMapping("MAgPIE_GGCMI.csv", type = "sectoral")
  area_mask <- toolAggregate(area_mask, rel = ggcmi_mapping, from = "MagPIE", to = "GGCMI", dim = 3, partrel = TRUE)
  ...
  cropcaldf <- toolAggregate(cropcaldf, rel = days_mapping, from = "day", to = "month", dim = 2, partrel = TRUE)
  } else if (frequency == "quarterly") {
    cropcaldf <- toolAggregate(cropcaldf, rel = days_mapping, from = "day", to = "quarter", dim = 2, partrel = TRUE)
...
if (products == "kcr") {
  ...
  iprod[, , "tece"] <- prod[, , "tece"] *
   ...
   cropcaldf <- toolAggregate(cropcaldf[, , cal_products_left], rel = ggcmi_mapping, from = "GGCMI", to = "MagPIE", dim = 3, partrel = TRUE)
