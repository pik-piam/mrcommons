...
readLPJmL_new <- function(subtype = "LPJmL4_for_MAgPIE_44ac93de:GSWP3-W5E5:historical:soilc") {
  ...
  subtype     <- toolSplitSubtype(subtype,
                                  list(version = NULL, climatemodel = NULL, scenario = NULL, variable = NULL))$variable
  ...
  .prepareLPJ <- function(datatype = numeric(),
    ...
    if (file_type == "clm") {
      ...
      startYear  <- out %>% str_subset("Output written in year:") %>% str_split(":") %>% unlist() %>%  str_trim() %>% subset(c(FALSE, TRUE)) %>% as.numeric()
      endYear    <- out %>% str_subset("Last year:") %>% str_split(":") %>% unlist() %>% str_trim() %>% subset(c(FALSE, TRUE)) %>% as.numeric()
