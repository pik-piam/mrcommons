...
#' @param aggregation "none", "k", or "springmann" the last uses Marco Springmann's custom product mapping
...
calcFAOTradePrices <- function(aggregation = "k") {
  ...
  if (aggregation == "k"){
    ...
    trade_agg <- trade
  } else stop("Only none k and springmann aggregations currently")
