...
calcEmiMacCEDS <- function(baseyear = 2020) {
  ...
  # n2o <- fullceds[,,"n2o_n"] * 44 / 28 # MtN to MtN2O
  ch4 <- fullceds[,,"ch4"] # Already in MtCH4

  co2cement <- dimSums(co2[, , c("2A1_Cement-production", "2A2_Lime-production", "2Ax_Other-minerals")], dim = 3)
  getNames(co2cement) <- "co2cement_process"
  ch4wsts  <- dimSums(ch4[, , c("5D_Wastewater-handling")], dim = 3)
  getNames(ch4wsts) <- "ch4wsts"
  ch4wstl  <- dimSums(ch4[, , c("5A_Solid-waste-disposal", "5C_Waste-combustion", "5E_Other-waste-handling")], dim = 3)
  getNames(ch4wstl) <- "ch4wstl"
  n2owaste <- dimSums(n2o[, , c("5A_Solid-waste-disposal", "5C_Waste-combustion", "5D_Wastewater-handling", "5E_Other-waste-handling")], dim = 3)
  getNames(n2owaste) <- "n2owaste"
  n2otrans <- dimSums(n2o[, , c("1A3b_Road", "1A3c_Rail", "1A3di_Oil_Tanker_Loading", "1A3dii_Domestic-navigation", "1A3eii_Other-transp")], dim = 3) # CEDS only reports 1A3aii_Domestic-aviation at the global level 
  getNames(n2otrans) <- "n2otrans"
  n2oacid <- dimSums(n2o[, , c("2B_Chemical-industry", "2B2_Chemicals-Nitric-acid", "2B3_Chemicals-Adipic-acid")], dim = 3) 
  getNames(n2oacid) <- "n2oacid"


  # Can't provide LUC CO2 from CEDS2024, so read EDGAR and fill it with 2005. 
  # TODO: Get a source for this with the same baseyear 
  ...
  euCountries <- c("ALA", "AUT", "BEL", "BGR", "HRV", "CYP", "CZE", "DNK", "EST", "FRO",
  ...
  if (baseyear %in% getYears(baselineEurostat)) {
    ...
    n2oacid[euCountries, baseyear, "n2oacid"] <- dimSums(baselineEurostat[euCountries, baseyear, c("n2oadac", "n2onitac")])
  ...
  return(list(x = x,
              weight = NULL,
              unit = "GtC, MtCH4, MtN",
              description = paste0("emissions in ",baseyear, " from CEDS, also 2005 LUC CO2 emissions from EDGAR"),
