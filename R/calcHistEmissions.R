...
calcHistEmissions <- function(subtype = "sector") {
  ...
  if (subtype == "sector") {
    ...
    gwp_ch4_eurostat <- 25 # values from AR4
    gwp_n2o_eurostat <- 298 # values from AR4
    emi[, , grep("ch4", getNames(emi), value = TRUE)] <- emi[, , grep("ch4", getNames(emi), value = TRUE)] / gwp_ch4_eurostat
    emi[, , grep("n2o", getNames(emi), value = TRUE)] <- emi[, , grep("n2o", getNames(emi), value = TRUE)] / gwp_n2o_eurostat
    # list of countries with eurostat data
    c_eurostat <- where(emi > 0)$true[]$regions
    c_noneu <- setdiff(getRegions(emi), c_eurostat)
    ...
    emi_ot <- emi_ot[, , c(".CO2.kt", ".CH4.kt", ".N2O.kt"), invert = TRUE]
